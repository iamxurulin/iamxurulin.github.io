(() => {
  ...
  var v = class {
      ...
      async loadModelList() {
        try {
          ...
          console.log("model_list.json 加载成功");
        } catch (err) {
          console.error("model_list.json 加载失败，使用默认 Natori", err);
          // 强制使用 Natori 作为兜底
          this.modelList = {
            models: [["Natori"]],
            messages: [["你好，我是xurulin"]],
          };
        }
      }
  ...
  var U = {
      ...
      info: {
        icon: b,
        callback: () => {
          open("https://github.com/iamxurulin/iamxurulin.github.io");
        },
      },
      photo: {
        icon: T,
  ...
  function E(e) {
    ...
    (function () {
      // 强制锁定为 Natori 模型（modelId=0, textureId=0）
      // 同时清除可能残留的旧 localStorage 值，防止干扰
      ...
      new Promise((resolve, reject) => {
        ...
        // 强制加载第 0 组第 0 个模型（就是 Natori）
        o.loadModel(0, 0, "你好，我是xurulin"); // 这里的欢迎语来自你的 model_list.json
      });

      // 加载 waifu-tips.json 并应用提示逻辑
      fetch(e.waifuPath)
